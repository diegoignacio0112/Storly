...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (reserva.estado !== 'en_uso') {
      return NextResponse.json({ error: 'El check-out solo está disponible mientras el espacio está en uso' }, { status: 400 })
    }

    const result = await pool.query(
      `UPDATE reservas SET checkout_fotos = $1, checkout_fecha = NOW(), checkout_notas = $2, estado = 'finalizada'
    ...
    await pool.query(
      `INSERT INTO notificaciones (usuario_id, tipo, titulo, mensaje, espacio_id, reserva_id)
       VALUES ($1, 'checkout_realizado', 'Check-out realizado', $2, $3, $4)`,
      [
        reserva.propietario_id,
        `${session.user.name} realizó el check-out de "${reserva.titulo}". Revisa la evidencia final`,
        reserva.espacio_id,
        id,
      ]
    )
